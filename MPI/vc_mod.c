...
int main(int argc, char** argv) {
  ...
  if (rank == 0) {
    ...
    MPI_Send(&vowels, 1, MPI_INT, 2, 0, MPI_COMM_WORLD);
    MPI_Send(&consonants, 1, MPI_INT, 3, 0, MPI_COMM_WORLD);
  }
    ...
    MPI_Recv(&consonants, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
